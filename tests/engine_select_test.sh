...
run() {  # run() <SOLARUS_ENGINE value or ""> [SOLARUS_ENGINE2_STOCK] -> launcher + engine output
  _eng="$1"; _stock="${2:-}"
  # A STOCK 2.x launch (SOLARUS_ENGINE=2 + SOLARUS_ENGINE2_STOCK=1) always CAPTURES
  # the engine's stdout/stderr to $SOLARUS_DIAG_LOG — that log is the only way to
  # observe an engine that draws nothing — so point it at a temp file and fold it
  # back in, otherwise the engine's output would not appear on this stdout at all.
  # The FABRIC legs do not capture, so the file simply stays empty there.
  : > "$TMP/diag.log"
  env -i PATH="$PATH" HOME="$TMP/home" \
      ...
      ${_stock:+SOLARUS_ENGINE2_STOCK="$_stock"} \
...
# The launcher cd's to $GAMEDIR and execs the SHIPPING engine as `./solarus-run`,
# so the fake engine's $0 is "./solarus-run" under dash but the resolved absolute
# path under some shells (macOS /bin/sh). Match either, or this test reports a
# launcher bug on a developer machine where the launcher is fine.
...
# T1b: default must NOT put v2 on the library path. Match the v2 dir EXACTLY, not
# a bare "v2" anywhere in the line — mktemp -d on macOS produces paths like
# /var/folders/sc/v2bl8sw.../T/..., which a loose match reports as a leak.
if echo "$out" | grep -q "LDPATH=.*$GAMEDIR/v2"; then
  bad "T1b default leaked v2 onto LD_LIBRARY_PATH" "$out"
else
  ok "T1b default keeps v2 off LD_LIBRARY_PATH"
fi

# T2: SOLARUS_ENGINE=2 -> the v2 binary, v2 libs FIRST, and — since the 2.x line
# gained the fabric renderer — the SAME blitter env as the shipping engine.
out=$(run 2)
...
if echo "$out" | grep -q "BLITTER=1"; then
  ok "T2c engine=2 (fabric, the default) exports the blitter env"
else
  bad "T2c engine=2 blitter env" "$out"
fi

# T2d: SOLARUS_ENGINE2_STOCK=1 says the deployed v2 tree is PRISTINE upstream —
# it has no blitter code, so exporting the flags would make the log read as if the
# fabric path were live. That leg must run the v2 binary with the env OFF.
outs=$(run 2 1)
if echo "$outs" | grep -q "RAN=$GAMEDIR/v2/solarus-run" && echo "$outs" | grep -q "BLITTER=unset"; then
  ...
  bad "T2d engine=2 stock leg" "$outs"
fi

# T2e: and that leg must capture the engine's output — a stock 2.x build draws
# nothing, so an uncaptured run produces no evidence at all. run() folds the diag
# log back in, so the engine's own lines only appear here if the capture happened.
if echo "$outs" | grep -q "DIAG capture ->" && echo "$outs" | grep -q "^ARGS="; then
  ok "T2e engine=2 stock leg captures the engine log"
else
  bad "T2e engine=2 stock capture" "$outs"
fi
...
out=$(run 99)
if ran_ship "$(echo "$out" | grep '^RAN=')"; then
