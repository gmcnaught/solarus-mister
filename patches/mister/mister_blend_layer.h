 ...
 * there is no first-wins lock to strand. Only the source-over (alpha-blend)
 * case is captured: a full-screen OPAQUE draw (NONE) is a promote (handled by
 * the menu-alias path), and full-screen ADD/MULTIPLY must NOT be captured —
 * they are not source-over so emitting them as PALPHA would be wrong. */
