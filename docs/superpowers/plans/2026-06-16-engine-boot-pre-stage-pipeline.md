...
- [x] **P2.1 — SDRAM-VRAM mode wiring. DONE (commit d7ec570, type-checked).** `SOLARUS_SDRAM_SRC` calls `blt_sdram_init(&em, SDRAM_ATLAS_BASE, SDRAM_CAP-SDRAM_ATLAS_BASE)` — atlas allocator based at 16MB, above the fixed bg-cache SDRAM region (no collision). `C_SRCSEL=1` publish already matches `sdram_src`.
- [x] **P2.2 — Lazy staging decoupled. DONE (commit d7ec570, type-checked).** Both lazy stage sites use `blt_stage_surface(&em, &ref)` (distinct per-surface SDRAM offset; idempotent re-stage; recorded in the `handles` map). `blt_sdram_free` added alongside the DDR3 free on evict/dirty. bg-cache keeps its fixed-offset `blt_stage` (below the atlas base). This is **Approach B (lazy-persistent)** — fully working contention fix; sources read from SDRAM. On-device validated under #34.

- [ ] **P2.3 — Boot bulk pre-stage (Approach A). NOT a self-contained change — needs the engine build + device (investigated 2026-06-17).** Approach A's only gain over the shipped P2.2/B is "no first-visit staging mid-scene"; it requires real engine work that is type-checkable but not logic-verifiable off-device:
  - **Hook layer:** the renderer (graphics layer) cannot reach `ResourceProvider` — only `MainLoop::get_resource_provider()` can. So drive the pre-load from a `build_engine.sh` sed-injection into `MainLoop::run()` (after `game` is set, before the loop), calling a new free fn `Solarus::mister_blitter_preload(ResourceProvider&)` (mirror the existing `mister_tag_camera_surface` free-fn pattern). That fn does `dynamic_cast<MisterBlitterRenderer*>(&SDLRenderer::get())` and a new public `preload_atlas(rp)` method.
  - **Must use the SHARED caches (else the renderer's pointer-keyed cache misses → re-stages anyway):** tilesets via `rp.get_tileset(id).get_tiles_image()` / `get_entities_image()` (enumerate ids with `CurrentQuest::get_resources(ResourceType::TILESET)`); sprites via `Sprite::get_animation_set(id)`. Do NOT `new Tileset(id)` (different Surface object than runtime).
  - **Sprites need a core accessor:** `SpriteAnimationSet::animations` + `SpriteAnimation::src_image` are private. Add a public `get_source_images()` (returns the distinct `SurfacePtr`s, skipping `src_image_is_tileset`) — a small core patch via `build_engine.sh`.
  - **Dedicated boot submit loop:** can't just call `upload()` in a loop — STAGE commands must be flushed to the fabric and the command ring + DDR3 bounce can't hold the whole atlas at once. Need a begin_frame → emit STAGE(s) up to ring/heap capacity → submit → wait C_DONE → `blt_emitter_free` the DDR3 bounces → repeat, all under the boot black screen. This is the main new logic and the main on-device risk (ring/heap sizing, fabric handshake).
  - **Gate behind `SOLARUS_SDRAM_PRELOAD`** (separate from `SOLARUS_SDRAM_SRC`) so it's opt-in and can't destabilize boot; measure boot time + staged-atlas size in #34.
  - Concrete Solarus API (verified in `work/solarus`):
