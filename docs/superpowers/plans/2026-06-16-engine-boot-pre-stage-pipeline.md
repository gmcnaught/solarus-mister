...
- [ ] **P2.3 — Boot bulk pre-stage (Approach A).** At quest mount, enumerate every staging-eligible static image source from the resource DB (`project_db.dat`: tileset images + sprite sheets; NOT runtime-rendered text per the spec's eligibility rule), decode each to RGB565, `blt_upload` into a DDR3 *bounce*, `blt_stage_surface`, then `blt_emitter_free` the DDR3 bounce (so the 16MB DDR3 heap holds one source at a time; the resident set lives in 64MB SDRAM). Run before the first gameplay frame (boot/black screen).
