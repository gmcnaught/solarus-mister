...
**CONTROLLER CORRECTION (2026-07-22) — `vram_demux` IS live, this note's original claim was wrong.** The original audit text here stated `vram_demux` is "not instantiated anywhere in the current build / dead code." That was a **false alarm from a broken grep**: the search pattern `"vram_demux ("` cannot match the real instantiation, which is `vram_demux vdemux` (module name + instance name `vdemux` + newline + `(`) at **`fpga/Solarus.sv:687`**. The module is **live and load-bearing**: `Solarus.sv:609/611` wire the blitter's `mem_dout`/`mem_busy` to the demux's `blt_dout`/`blt_busy` (so *every* blitter DDR read returns through it), and its DDR side (`bd_*`) feeds `ddr_blitter_arb` for the command-ring / `BLT_OP_STAGE` / GRID_BUF / TL_BUF reads. What IS dead is only the demux's **SDRAM-FB leg** (`sd_*`/`dst_*` → the P_DST ch0 cache), because the framebuffer moved on-chip to `comp_fbram` (PR #49) — that FB→SDRAM leg gets no live traffic. That dead leg is exactly what Task 3 removes when it retargets the FB address region to DDR3. The `` `FB_DDR0_QW ``/`` `FB_DDR1_QW `` macros in `vram_demux.sv` are the correct, live FB-routing decode to modify — not dead code to ignore.
