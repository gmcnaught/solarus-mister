...
| `SOLARUS_RINGDBUF` | **Default ON since 2026-07-26 (`=0` disables the overlap). Requires a bank-1-capable RBF — and so does `=0`, see the matched-pair warning below.** Ring command-bank double-buffer: lets the A9 build frame S+1's command list into command bank `(S+1)&1` while the fabric still composites frame S out of the other bank, instead of the two serializing on one shared ring. On: `ensure_frame()`'s handshake becomes a 2-deep fence (waits for `C_DONE >= submit_seq-1`, wrap-safe signed compare, instead of `== submit_seq`), the per-frame control words (`C_CMDCOUNT`/`C_TARGET`/`C_CLEAR`/`C_FLAGS`/`C_SRCSEL`) write to the building frame's bank base (bank 1 = `+OFF_CTRL1`) while the `C_SUBMIT`/`C_DONE` doorbell stays GLOBAL at bank 0, `BANK_EN` (`C_SUBMIT`'s high word) is written once at init, DDR-bounce frees go through a deferred queue drained once the fabric's `C_DONE` proves the tagged frame finished, and a scene/tileset transition does one deliberate full pipeline drain (`drain_pipeline()`) before rewriting the shared FRT/CFT/CLUT/GRID_BUF tables (and before every `blt_heap_reset`) so an in-flight frame in the other bank never resolves against content that's mid-rewrite. **TL_BUF is never bank-split** ([correction](superpowers/specs/2026-07-26-ring-double-buffer-design.md) §4.1): its only writer, `res_arm_`, is a per-scene rebuild that begins with a full `drain_pipeline()`, so no frame is ever in flight while it rewrites TL_BUF — it stays full-width (`tl_cap`) and cursor-at-0 in every bank, on or off. Only `SP_BUF` is genuinely per-frame and actually halves (`sp_frame_cap`). **Off disables the OVERLAP only**: bank 0 only, `BANK_EN=0`, the original `done == submit_seq` fence, `sp_frame_cap` at full width (`== sp_cap`), and immediate (non-deferred) frees — byte-identical per-frame behaviour to the pre-branch engine *on the same bitstream*. **It is NOT an old-RBF compat leg** (an earlier revision of this table said it was, wrongly): `OFF_HEAP` moved `0x80000`→`0x100000` **unconditionally** to make room for bank 1 and the fabric's `SRC_QW` moved in lockstep, so this engine reads every `OP_STAGE` source from the new base whatever the flag says. Against a pre-ring-dbuf bitstream every atlas is fetched 512 KiB low → silently garbage tiles, flag on or off, with no version handshake to catch it. Engine and RBF are a matched pair and roll back as a pair. `SOLARUS_BLITTER_DIAG`'s `[blitter timing]` banner's `fabric=` column IS the A9's `ensure_frame()` wait — the single C_DONE spin site, on or off — so it is the number to A/B: compare `fabric=` with `SOLARUS_RINGDBUF=0` vs `=1` (it should shrink once the two banks overlap). There is deliberately no separate `fence=` column (same spin site, it would just duplicate `fabric=`); the banner does add `ringdbuf=on/off` and `dfq_drop=N` (deferred-free-queue entries dropped because the queue filled — a nonzero count means DDR3 bounce bytes are leaking until the next `blt_heap_reset`). |
