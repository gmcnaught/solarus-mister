...
| `SOLARUS_BGPLANE` | Per-**layer** background-plane bake: each map layer with static (non-animated) tile content is baked ONCE per map/tileset change into its own permanent SDRAM plane (**ARGB4444**, real per-pixel alpha via the fabric's `bgplane_coverage` tracker), replacing that layer's per-frame `BLT_OP_TILELIST` static replay with one `BLT_BLEND_PALPHA` COPY. Applies to every layer with static content, not just the map's base layer — a layer whose SDRAM allocation fails (or that has no static content) falls back to the ordinary per-bucket replay for that layer only, unaffected by any other layer's plane. No `scroll_ratio` (parallax) disqualification: a parallax pattern sharing a layer with static ground tiles no longer disables baking for that layer (the ARGB4444 alpha=0 gaps leave whatever's underneath untouched — the fix for Mystery of Solarus DX map 119). **HW-validated**, incl. graceful fallback: an interior map (376×248) baked its one layer cleanly (`fabric_hw` ~6.1ms, ~51fps); an overworld map (1152×1040, 3 static layers) baked 1 of 3 (the other 2 hit the perm-SDRAM limit below and fell back, no crash/corruption, HUD intact — `fabric_hw` ~10ms, ~28fps, A9-bound). **Known limit:** permanent SDRAM is shared with the whole-quest atlas preload (#66); on a large map (like that overworld: ~60 MiB atlas, ~4 MiB perm headroom) not every layer-plane fits, so some fall back to per-bucket replay — correct but not accelerated. Follow-up to widen the perm-SDRAM budget for this case is tracked separately (not yet fixed). Opt-in, default OFF pending broader HW soak. See `docs/frame-dataflow.md`. |
