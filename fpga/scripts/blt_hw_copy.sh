...
#
# Regions: ctrl/ring @ 0x3A070000 (BLTCTRL_QW 0x0740E000); source @ 0x3A078000
# (SRC_QW 0x0740F000); framebuffer BUF0 @ 0x3A000040.
set -e

echo "== upload 4x2 source sprite @ 0x3A078000 (stride 8B) =="
# row0: red, green, blue, white
busybox devmem 0x3A078000 32 0x07E0F800   # [green | red]
busybox devmem 0x3A078004 32 0xFFFF001F   # [white | blue]
# row1: yellow, magenta, cyan, gray
busybox devmem 0x3A078008 32 0xF81FFFE0   # [magenta | yellow]
busybox devmem 0x3A07800C 32 0x841007FF   # [gray | cyan]
...
echo "== BLIT command @ ring 0x3A070040: op=BLIT src_off=0 w=4 h=2 stride=8 dst=(100,80) =="
busybox devmem 0x3A070040 32 0x00000003   # qw0 lo: opcode=BLIT, blend=COPY
busybox devmem 0x3A070044 32 0x00000000   # qw0 hi: src_off=0
busybox devmem 0x3A070048 32 0x00000008   # qw1 lo: src_x=0 | stride=8
busybox devmem 0x3A07004C 32 0x00020004   # qw1 hi: h=2 | w=4
busybox devmem 0x3A070050 32 0x00000000   # qw2 lo: src_y=0
busybox devmem 0x3A070054 32 0x00500064   # qw2 hi: dst_y=80 | dst_x=100
busybox devmem 0x3A070058 32 0x00000000   # qw3 lo
busybox devmem 0x3A07005C 32 0x00000000   # qw3 hi
busybox devmem 0x3A070060 32 0x00000001   # cmd1 END
...
busybox devmem 0x3A070020 32 0x00000000   # flags=0 (NO clear)
busybox devmem 0x3A070028 32 0x00000000   # done_seq=0
busybox devmem 0x3A070000 32 0x00000001   # submit_seq=1 (doorbell)
sleep 1

echo "== readback =="
echo "done@28 : $(busybox devmem 0x3A070028 32)  (expect 00000001)"
