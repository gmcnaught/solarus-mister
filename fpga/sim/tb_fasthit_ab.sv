// tb_fasthit_ab.sv — A/B of the same-block fast hit path (FASTHIT).
//
// Two INDEPENDENT stacks, both with SRC_EARLY=1, differing only in SRC_FASTHIT,
// each driven by an identical copy of comp_pipeline's F_WALK issue logic.
//
//   OFF: a hit costs 3 controller cycles -- S_IDLE (take, address the tag RAM),
//        S_LOOKUP (hit_blk_now out, address the data RAM), S_RD_RESP (req_q out).
//   ON : the block that served the last read is remembered, so on a (tag,set)
//        match the data RAM is addressed in the request cycle and S_LOOKUP is
//        skipped -- 2 controller cycles.
//
// The point of interest is the WARM leg: early restart already covers cold spans,
// and this is the lever for the 97.4%-of-reads hit path.
//
// The walk deliberately spans FOUR times the 32 KB cache and is then REPLAYED, so
// blocks are evicted and re-filled under the predictor. That is the case that
// would expose a stale fh_* mapping, and the two legs are compared beat for beat.
`timescale 1ns/1ps

module tb_fasthit_ab #(parameter integer INIT_WAIT_SIM = 40);

localparam integer PERIOD = 10;
localparam integer NLBQ   = 1024;  // linebuf qwords per walk (= 4096 px PAL8)
...
`define STACK(NAME, EARLY)                                                      \
    ...
    sdram_fb_cache #(.INIT_WAIT_SIM(INIT_WAIT_SIM), .SRC_BLOCKS(128),           \
                     .SRC_EARLY(1), .SRC_FASTHIT(EARLY)) u_``NAME (                              \
...
initial begin
    ...
    $display("");
    $display("=== Fast-hit A/B — %0d linebuf qwords (= %0d px) per walk ===", NLBQ, NLBQ*4);
    $display("Two independent stacks, both SRC_EARLY=1, SRC_FASTHIT=0 vs 1,");
    $display("identical PAL8 F_WALK driver on each.");
    $display("");

    do_walk(32'h04000, t_a_cold, t_b_cold);
    $display("  COLD  FASTHIT=0: %5d cyc = %0.3f cyc/px", t_a_cold, t_a_cold*1.0/(NLBQ*4));
    $display("  COLD  FASTHIT=1: %5d cyc = %0.3f cyc/px   -> %0.2fx",
             t_b_cold, t_b_cold*1.0/(NLBQ*4), t_a_cold*1.0/t_b_cold);
    // evict everything (4x the cache), then come back -- the stale-mapping case
    do_walk(32'h20000, t_a_cold, t_b_cold);
    do_walk(32'h40000, t_a_cold, t_b_cold);
    do_walk(32'h60000, t_a_cold, t_b_cold);
    do_walk(32'h04000, t_a_warm, t_b_warm);
    $display("  RE-WALK (post-eviction) FASTHIT=0: %5d cyc = %0.3f cyc/px", t_a_warm, t_a_warm*1.0/(NLBQ*4));
    $display("  RE-WALK (post-eviction) FASTHIT=1: %5d cyc = %0.3f cyc/px   -> %0.2fx",
             t_b_warm, t_b_warm*1.0/(NLBQ*4), t_a_warm*1.0/t_b_warm);
    $display("");
    $display("  read-data mismatches between legs: %0d", mismatches);
    $display("");
    if (mismatches != 0) begin
        $display("RESULT: FAIL (fast hit returned different data)");
        $finish;
    end
    $display("RESULT: PASS");
    $finish;
