...
module tb_blitter_palpha;
  ...
  initial begin
    ...
    mem[32'h1C000]=64'd1; mem[32'h1C001]=64'd2; mem[32'h1C002]=64'd0;
    ...
    mem[32'h1C008]={32'd0, 8'd0,8'd1,8'd3,8'd3};   // op=BLIT(3) blend=PALPHA(3) fmt=ARGB4444(1)
    mem[32'h1C009]={16'd2,16'd2,16'd0,16'd4};       // h=2 w=2 src_x=0 stride=4
    mem[32'h1C00A]={16'd70,16'd70,16'd0,16'd0};     // dst=(70,70) src_y=0
    mem[32'h1C00B]={16'd0,16'd0,8'd0,16'd0};        // (no key/alpha)
    mem[32'h1C00C]=64'd1;                           // cmd1 END
    // ARGB4444 source @ SRC (0x1D000): 2x2, stride 4B. With stride=4 the whole
    // 2x2 surface is 16 bytes = TWO qwords' worth but PACKED: row0 at bytes 0..3
    // (qw 0x1D000 [31:0]), row1 at bytes 4..7 (qw 0x1D000 [63:32]). So a single
    // qword holds the entire sprite: {px(1,1),px(0,1),px(1,0),px(0,0)}.
    mem[32'h1D000]={SP11, SP01, SP10, SP00};
  end
