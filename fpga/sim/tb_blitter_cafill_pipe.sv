// tb_blitter_cafill_pipe.sv — CONST_ALPHA (blend_mode=2) FILL must blend like the
// C golden blt_blend565, exactly as a CONST_ALPHA BLIT does, through the
// comp_pipeline burst path (C_PIPE=1).
//
//   out_ch = div255_round(src_ch*a + dst_ch*(255-a))   [== blt_blend565]
//
// WHY: a colored fade (TransitionFade, the default) overlays a translucent black
// rect via Surface::fill_with_color -> renderer.fill(..., BlendMode::BLEND) with
// the colour's alpha < 255. The fabric FILL datapath previously mapped any FILL
// whose blend was not ADD/MULTIPLY to COMP_COPY (opaque), so the overlay wrote
// the raw colour at full strength -> the whole screen went solid black for the
// entire fade (the "extra black frames"). This gate locks the const-alpha FILL
// onto the same divide-free blend as the const-alpha BLIT.
//
// Harness mirrors tb_blitter_blend_pipe / tb_blitter_add_pipe (DDR mem + p0_*
// SDRAM source + on-chip comp_fbram dest). CLEAR paints BG, then a CONST_ALPHA
// BLIT (regression anchor — already passes) and a CONST_ALPHA FILL (the new
// behaviour) composite over BG; both must equal ref_blend().
`timescale 1ns/1ps
`default_nettype none
`include "blitter_defs.vh"
module tb_blitter_cafill_pipe;
  localparam [28:0] WBASE = 29'h07400000;
  localparam        MEMQW = (`SRC_QW - 29'h07400000) + 29'h10000;  // [#52] tracks SRC_QW heap base
  localparam [15:0] BG = 16'h8410, REDS = 16'hF800;
  localparam [7:0]  AL = 8'd128;   // const alpha
  ...
  // reference divide-free const-alpha blend (matches blt_blend565)
  ...
  initial begin
    ...
    // cmd0 CONST_ALPHA BLIT (anchor): blend=2, src_off=0, w=2 h=2 stride=4, dst=(60,60), alpha=AL
    mem[32'h200008]={32'd0, 8'd0,8'd0,8'd2,8'd3};       // op=BLIT(3) blend=ALPHA(2)
    mem[32'h200009]={16'd2,16'd2,16'd0,16'd4};          // h=2 w=2 src_x=0 stride=4
    mem[32'h20000A]={16'd60,16'd60,16'd0,16'd0};        // dst=(60,60) src_y=0
    mem[32'h20000B]={16'd0,16'd0,8'd0,AL,16'd0};        // alpha=AL ([23:16]) — full 64b
    // cmd1 CONST_ALPHA FILL: blend=2, color=REDS, w=2 h=2, dst=(50,50), alpha=AL
    mem[32'h20000C]={32'd0, 8'd0,8'd0,8'd2,8'd2};       // op=FILL(2) blend=ALPHA(2)
    mem[32'h20000D]={16'd2,16'd2,16'd0,16'd0};          // h=2 w=2
    mem[32'h20000E]={16'd50,16'd50,16'd0,16'd0};        // dst=(50,50)
    mem[32'h20000F]={16'd0, REDS, 8'd0, AL, 16'd0};     // color=REDS ([47:32]) alpha=AL ([23:16]) — full 64b
    mem[32'h200010]=64'd1;                               // cmd2 END
    // CONST_ALPHA blit source @ SRC (0x201000): 2x2 solid REDS (stride 4B -> one qword)
    mem[(SRC_WIN + 29'h00)]={REDS,REDS,REDS,REDS};
  end
  ...
  initial begin
    ...
    ckpix(60,60, ref_blend(REDS, BG, AL), "ca-blit");
    ckpix(61,61, ref_blend(REDS, BG, AL), "ca-blit");
    ckpix(50,50, ref_blend(REDS, BG, AL), "ca-fill");
    ckpix(51,51, ref_blend(REDS, BG, AL), "ca-fill");
