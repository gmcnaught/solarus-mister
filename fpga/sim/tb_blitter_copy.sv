...
module tb_blitter_copy;
  ...
  initial begin
    ...
    mem[32'hE000]=64'd1;   // submit=1
    mem[32'hE001]=64'd2;   // cmd_count=2 (BLIT + END)
    mem[32'hE002]=64'd0;   // target_buf=0 (BUF0)
    mem[32'hE003]=64'd0;   // clear_color
    mem[32'hE004]=64'd0;   // flags = 0 (NO clear)
    mem[32'hE005]=64'd0;   // done=0
    // cmd0 BLIT @ ring 0xE008 : op=3, src_off=0, stride=16, w=8, h=4, dst=(20,10)
    mem[32'hE008]=64'h0000_0000_0000_0003;                 // opcode=BLIT, blend=COPY
    mem[32'hE009]={16'd4,16'd8,16'd0,16'd16};               // h=4 w=8 src_x=0 stride=16
    mem[32'hE00A]={16'd10,16'd20,16'd0,16'd0};              // dst_y=10 dst_x=20 src_y=0
    mem[32'hE00B]=64'd0;
    ...
    for(y=0;y<4;y=y+1) for(x=0;x<8;x=x+1)
      mem[32'hF000 + y*2 + (x>>2)][(x%4)*16 +: 16] = 16'h1000 + y*8 + x;
  end
  ...
  initial begin
    ...
    to=0;
    while (mem[32'hE005][31:0] !== mem[32'hE000][31:0] && to<2000000) begin @(posedge clk); to=to+1; end
